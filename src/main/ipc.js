...
const { fetchDeckFromUrl, fetchCardDetails, fetchCardPrice, scoreNameMatch } = require('./scraper');
...
function registerIpcHandlers(ipcMain, db, jobRunner, logger) {
  ...
  ipcMain.handle('portfolio:refresh', async () => {
    const cards = db.prepare('SELECT id, passcode, en_name, de_name FROM cards ORDER BY id ASC').all();
    if (cards.length === 0) {
      return { updated: 0 };
    }
    const settings = db.prepare('SELECT key, value FROM settings').all();
    ...
    let updated = 0;
    for (const card of cards) {
      const priceResult = await fetchCardPrice({
        passcode: card.passcode || '',
        en_name: card.en_name || '',
        de_name: card.de_name || '',
        userAgent
      }).catch((error) => {
        logger.log(db, 'error', `Price fetch failed for ${card.passcode || card.en_name || card.de_name}: ${error.message}`);
        return null;
      });
  ...
  ipcMain.handle('portfolio:get', () => {
    const cards = db.prepare(`
      SELECT id, de_name, en_name, passcode, price_value, price_currency, price_source, price_updated_at
      FROM cards
    ...
    const totals = cards.reduce((acc, card) => {
      ...
      acc[currency] = (acc[currency] || 0) + Number(card.price_value || 0);
      return acc;
    }, {});
    const missing = db.prepare('SELECT COUNT(*) as count FROM cards WHERE price_value IS NULL').get();
    return {
      cards,
      totals,
      missing: missing?.count || 0,
      lastUpdatedAt: cards[0]?.price_updated_at || null
    };
  });
