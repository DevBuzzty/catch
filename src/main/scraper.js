...
async function fetchCardPrice({ passcode, en_name, de_name, userAgent }) {
  const normalizedPasscode = normalizeString(passcode);
  const normalizedEn = normalizeString(en_name);
  const normalizedDe = normalizeString(de_name);
  const query = normalizedPasscode || normalizedEn || normalizedDe;
  if (!query) {
    return { price: null, currency: null, source: null, name: null };
  }
  const url = normalizedPasscode
    ? `${YGOPRO_BASE}?id=${encodeURIComponent(normalizedPasscode)}`
    : `${YGOPRO_BASE}?name=${encodeURIComponent(query)}`;
  ...
  const cardmarketPrice = Number(prices.cardmarket_price);
  if (Number.isFinite(cardmarketPrice) && cardmarketPrice > 0) {
    return { price: cardmarketPrice, currency: 'EUR', source: 'cardmarket', name: card.name || null };
  }
  const tcgPlayerPrice = Number(prices.tcgplayer_price);
  if (Number.isFinite(tcgPlayerPrice) && tcgPlayerPrice > 0) {
    return { price: tcgPlayerPrice, currency: 'USD', source: 'tcgplayer', name: card.name || null };
  }
  const cardKingdomPrice = Number(prices.cardkingdom_price);
  if (Number.isFinite(cardKingdomPrice) && cardKingdomPrice > 0) {
    return { price: cardKingdomPrice, currency: 'USD', source: 'cardkingdom', name: card.name || null };
  }
  return { price: null, currency: null, source: null, name: card.name || null };
}
