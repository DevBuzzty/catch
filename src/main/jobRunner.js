...
const { fetchCardDetails, scoreNameMatch } = require('./scraper');
...
class JobRunner {
  ...
  async processCard(card, settings, force) {
    ...
    if (nameMismatch) {
      this.logger.log(this.db, 'warn', `Name mismatch for card ${card.id}: existing=${existingName}, fetched=${fetchedName}`);
    }
