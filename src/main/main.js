...
const { analyzeCardImage } = require('./imageScanner');
...
function createWindow() {
  ...
  mainWindow.webContents.on('did-finish-load', () => {
    mainWindow.webContents.send('scanner:status', { connected: scannerClients.size > 0, count: scannerClients.size });
  });
}
...
app.whenReady().then(() => {
  ...
  scannerServer = startScannerServer({
    ...
    onImage: async (payload, socket) => {
      ...
      try {
        ...
        const response = {
          ...
          card: {
            passcode: detail.passcode || passcode || '',
            en_name: detail.name || detail.en_name || name || '',
    ...
    onConnect: (socket) => {
      ...
      if (mainWindow) {
        mainWindow.webContents.send('scanner:status', { connected: true, count: scannerClients.size });
      }
    },
    onDisconnect: (socket) => {
